...
machine. Each block, the top `MaxValidators` (defined by `x/symstaking`) validators
...
The slashing module implements the `StakingHooks` defined in `x/symstaking` and are used as record-keeping of validators information. During the app initialization, these hooks should be registered in the staking module struct.
